...
def build_logic_app_payload(name: str, email: str, contact: str, query: str, support_agent_email: str = "") -> Dict[str, Any]:
    return {
...
@tool
def general_enquiry_tool(query: str, name: str = "", email: str = "", contact: str = "") -> Dict[str, object]:
    """Use this tool for general questions, informational requests, or low-risk support enquiries. It routes the request to a standard knowledge-base response workflow."""
    return {
        "tool_name": "General Enquiry Handler",
        "critical_level": "Low",
        "logic_app_url": "https://example.logic.azure.com/workflows/general-enquiry-flow/triggers/manual/paths/invoke",
...
class RequestAgent:
    def __init__(self) -> None:
        ...
        if AzureChatOpenAI is not None and AgentExecutor is not None and create_tool_calling_agent is not None and self.client is not None:
            ...
            prompt = ChatPromptTemplate.from_messages(
                [
                    ("system", "You are a helpful request-routing agent. Use exactly one of the provided tools to process the customer request. After invoking the tool, return the tool result as a JSON object with the keys tool_name, critical_level, classification, steps, draft_response, status, routing, and logic_app_url. Do not include extra explanatory text."),
                    ("human", "{input}"),
    ...
    def process_request(self, query: str, name: str, email: str, contact: str, support_agent_email: str = "deepak.passrid@gmail.com") -> Dict[str, Any]:
        ...
        if isinstance(parsed_output, dict):
            ...
            # If the agent already executed the tool, it may return the full result
            # including `logic_app_response` or `logic_app_payload`. In that case,
            # don't invoke the tool again to avoid duplicate Logic App calls.
            already_executed = (
                isinstance(parsed_output.get("logic_app_response"), dict)
                or isinstance(parsed_output.get("logic_app_payload"), dict)
                or parsed_output.get("status") in ("Received", "success")
            )

            if already_executed:
                full_result = parsed_output
            elif selected_tool in self.tool_lookup:
...
if submitted:
    ...
    st.info("Please wait — processing your request. This may take 1-2 minutes.")
    with st.spinner("Agent is choosing the workflow tool and invoking the Logic App — this may take 1-2 minutes..."):
        try:
    ...
    if isinstance(logic_app_response, dict):
